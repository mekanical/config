...
backFileName = ARGV[2] #nil #"back_thick2.bmp"

# fdtd paramater
$nx = 300; $ny = 300; $nz = 100; $nt = 40000
$dx = "0.1d-3"; $dy = "0.1d-3"; $dz = "0.1d-3"

$freqS_imp = "500.0d6"; $freqE_imp = "10.0d9"; $freqPoint_imp = "301"

# substrate paramater
dz = 0.1e-3
substrateDepth = 0.6e-3; substrateDepthCells = (substrateDepth/dz).round
epsR = 10.2; freq = 2.0e9; tanD = 0.005
substrateSigma = tanD * 2 * 3.141592653 * freq * 8.85418782e-12 * epsR

# feed paramater
$nfeed = 4
$innerR = "100.0d0"; $type = "1"
$tau = "800.0d0 * dt"
...
def outSetFeed(feedNum, startX, startY, startZ, endX, endY, endZ, amp)
  ...
  feedCode = "    call setfeed(" + feedNum.to_s + ", " + startX.to_s + ", " + startY.to_s + ", " + startZ.to_s +
...
def outSetPlane(materialNum, startX, startY, startZ, endX, endY, endZ)
  ...
  file = File.open($outFileName+"_check.txt", "a")
    case
    when startX == endX && startY < endY && startZ < endZ
      file.print(startX, " ", startY, " ", startZ, "\n")
...
def bmpToModel(modelImage, baseX, baseY, drawPosition)
  # 多次元配列確保
  ...
  imageData.length.times{ |x|
    imageData[x].length.times{ |y|
      ...
      case
      when tempColor[0] == 255 && tempColor[1] == 0 && tempColor[2] == 0 # red
        imageData[x][y] = 2

      when tempColor[0] == 255 && tempColor[1] == 255 && tempColor[2] == 0 # red+green
        imageData[x][y] = 3

      when tempColor[0] == 0 && tempColor[1] == 0 && tempColor[2] == 255 # blue
        imageData[x][y] = 4

      when tempColor[0] == 0 && tempColor[1] == 255 && tempColor[2] == 255 # blue+green
        imageData[x][y] = 5

      when tempColor[0] == 0 && tempColor[1] == 0 && tempColor[2] == 255 # green
        imageData[x][y] = 6

      when tempColor[0] == 255 && tempColor[1] == 0 && tempColor[2] == 255 # red+blue
        imageData[x][y] = 7

      when tempColor[0] >= 1 && tempColor[1] >= 1 && tempColor[2] >= 1 # white
        imageData[x][y] = 1
  ...
  imageData[0].length.times{ |y|
    ...
    if(tempImageData == 0)
      outSetPlane(2, baseX+tempX, baseY+y, drawPosition, baseX+modelImage.width, baseY+y+1, drawPosition)
    end
  }

  # Feed modeling
  feedNum = 1; feedFlag = false; optim = 0
  tempFeedColor = 0;
  feedCode = ""
  imageData.length.times{ |x|
    tempY = 0
    imageData[x].length.times{ |y|
      if(imageData[x][y] >= 2 && imageData[x][y] < 7 && !feedFlag)
        feedFlag = true
        tempY = y
        tempFeedColor = imageData[x][y]

      elsif(imageData[x][y] != tempFeedColor && feedFlag)
        x != imageData.length-1 ? optim = 0 : optim = 1
        case tempFeedColor
        when 2
          feedCode = feedCode + outSetFeed(feedNum, baseX+x+optim, baseY+tempY, drawPosition, baseX+x+optim, baseY+y, drawPosition, 1.0e0)

        when 3
          feedCode = feedCode + outSetFeed(feedNum, baseX+x+optim, baseY+y, drawPosition, baseX+x+optim, baseY+tempY, drawPosition, 1.0e0)

        when 4
          feedCode = feedCode + outSetFeed(feedNum, baseX+x+optim, baseY+tempY, drawPosition, baseX+x+optim, baseY+y, drawPosition, 0.0e0)

        when 5
          feedCode = feedCode + outSetFeed(feedNum, baseX+x+optim, baseY+y, drawPosition, baseX+x+optim, baseY+tempY, drawPosition, 0.0e0)
        end
    ...
    if(imageData[x][modelImage.height-1] != tempFeedColor && feedFlag)
      x != imageData.length-1 ? optim = 0 : optim = 1
      case tempFeedColor
      when 2
        feedCode = feedCode + outSetFeed(feedNum, baseX+x+optim, baseY+tempY, drawPosition, baseX+x+optim, baseY+modelImage.height, drawPosition, 1.0e0)

      when 3
        feedCode = feedCode + outSetFeed(feedNum, baseX+x+optim, baseY+modelImage.height, drawPosition, baseX+x+optim, baseY+tempY, drawPosition, 1.0e0)

      when 4
        feedCode = feedCode + outSetFeed(feedNum, baseX+x+optim, baseY+tempY, drawPosition, baseX+x+optim, baseY+modelImage.height, drawPosition, 0.0e0)

      when 5
        feedCode = feedCode + outSetFeed(feedNum, baseX+x+optim, baseY+modelImage.height, drawPosition, baseX+x+optim, baseY+tempY, drawPosition, 0.0e0)
      end
...
if(backFileName)
  backImage = BitMap.read(backFileName)
  if(topImage.width != backImage.width && topImage.height != backImage.height)
    print(topImage.width, ", ", backImage.width, "\n")
    print(topImage.height, ", ", backImage.height, "\n")
...
file = File.open($outFileName+".f90",'w')
  file.print("! Used Bitmap: '#{topFileName}' & '#{backFileName}'\n")
...
# top model draw
feedCode = bmpToModel(topImage, baseX, baseY, $baseZ+substrateDepthCells)
if(backFileName)
  bmpToModel(backImage, baseX, baseY, $baseZ)
end
